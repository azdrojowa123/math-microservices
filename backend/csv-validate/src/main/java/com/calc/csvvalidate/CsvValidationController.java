...
public class CsvValidationController {
    ...
    public ResponseEntity calculateCsvData(@RequestBody Map<Integer, Integer> csvData, @PathVariable(value = "periods") int periods) {
        ...
        if (validationService.validate(csvData, periods)) {
            return circuitBreaker.run(() -> survivalCurveClient.getSurvivalCurveResults(validationService.convertData(csvData), periods),
                    throwable -> ResponseEntity.status(HttpStatus.GATEWAY_TIMEOUT).body("Connection to server responsible for computation of the survival curve is not available"));
        }
    ...
    public ResponseEntity<Boolean> validateCsvData(@RequestBody Map<Integer, Integer> csvData, @PathVariable(value = "periods") int periods) {
